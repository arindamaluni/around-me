  ...
  useEffect(() => {
    return firebaseAuth.onAuthStateChanged((firebaseUser) => {
      ...
      const auth = firebaseUser ?
        { loggedIn: true, loginMethod:'email', uid, email, 
          displayName:getDisplayNameFromEmail(email), photoURL} :
        { loggedIn: false };
      ...
      if (uid) {
        ...
        loadProfile(uid).then(p => {
          profile=p
          if (!profile)
            profile = saveOrUpdateProfile({uid, email, 
              displayName:getDisplayNameFromEmail(email)});    
          store.dispatch(setProfile({profile}));
        })
      }
      console.log("useAuthInit -> stateChange ->User:", store.getState().authState)
    });
...
const getDisplayNameFromEmail = (name) => {
  ...
  const displayName = namePart.map(part =>
