...
const MapView = (props: any) => {
  ...
  useEffect(() => {
    ...
    if (!readOnly) getAddress(center.lat, center.lng, '');
  }, [center]);
  ...
  async function getAddress(
    ...
    if (!searchText || searchText.trim() === '') {
      ...
      const res = await fetch(
        `https://maps.googleapis.com/maps/api/geocode/json?latlng=${latitude},${longitude}&key=AIzaSyBsqyN1XfESBdJHJL58hJOLeDPCZH6Y9Hg`,
      );
      const reply = await res.json();
      // console.log(reply);
      setCurrentPick(oldState => {
      ...
      const res = await fetch(
        `https://maps.googleapis.com/maps/api/geocode/json?address=${encodeURI(
          searchText,
        )}&key=AIzaSyBsqyN1XfESBdJHJL58hJOLeDPCZH6Y9Hg`,
      );
      const reply = await res.json();
      // console.log(reply);
      setCurrentPick(oldState => {
  ...
  const resetMarkerAndCenter = location => {
    ...
    setCurrentPick(oldPick => {
      return {...oldPick, lat: location.lat, lng: location.lng};
    });
  };
        ...
        <IonContent>
            ...
            <>
              <IonItem className="geoAbs">
                ...
                <IonSelect
                  ...
                  onIonChange={e => {
                    /* console.log(e.detail.value) */ resetMarkerAndCenter(
                      e.detail.value,
                    );
                  }}
                >
          ...
          <IonFab vertical="center" horizontal="end">
            ...
            <IonFabButton
              size="small"
              onClick={() => {
                /* console.log(location) */ onClose(location);
              }}
            >
              <IonIcon
                md={arrowBackSharp}
