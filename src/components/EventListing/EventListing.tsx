      ...
      return (
        <IonCard color="light" style={{padding:"5px"}} key={event.id}>
          ...
          <div style={{position:"relative"}}>      
            ...
            <div style={{position:"absolute", bottom:"2px", backgroundColor:"#FF0077", opacity:"0.65", color:"white", width: "100%", textAlign:"center", padding:"3px",  fontWeight:"bold"}}>{event.overlay}</div>
          </div>
          <IonCardHeader>
            <IonCardSubtitle>
              <div style={{display: "flex", flexDirection:"row", justifyContent:"space-between", width: "100%"}}>
                <span style={{fontSize:"14px", color:"#FF0077", display: "inline"}}>{event.highlight}</span>
                <span style={{fontSize:"14px", color:"#FF0077", display: "inline"}}>{moment.utc(event.date).format('ll')}</span>
              </div>
            </IonCardSubtitle>
            <IonCardTitle ><div className = "ion-text-center" ><h2 style={{fontWeight: "bold"}}>{event.title}</h2></div></IonCardTitle>          
          </IonCardHeader>

          {event.externalLink && 
            <div style={{display:"flex", fontSize:"10px", color:"#FF0077", justifyContent:"center"}}>
          ...
          <IonFooter>
            <div style={{display:"flex", justifyContent:"space-around"}}>
              <button style={{backgroundColor:"transparent", outline:"none"}}>
                <IonIcon style={{fontSize:"25px"}} color="primary" md={thumbsUpSharp} ios={thumbsUpOutline} />
